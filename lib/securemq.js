...
function bind(options) {
  preview('bind called');
  var settings = setSettings(clone(options));
...
function newServiceBind(service, pattern, _options) {
  ...
  self.service = clone(service);
  self.pattern = clone(pattern); 
  
  var _settings = clone(settings);
  //preview('settings', _settings);
  
  var xsettings = {};
  xsettings.protocol = clone(_settings.protocol);
  xsettings.port = clone(_settings.port);
  xsettings.hwm = clone(_settings.hwm);
  
  var i = checkService(service, pattern);
  if ( i === false ) {
    var i = addService({ name: self.service, type: 'bind', pattern: self.pattern, queue: [], n: 0, xsettings: xsettings, self: self });     // add new service
  }
  self.i = i;
  ...
  function onconnect(req, res) {
    ...
    preview('newServiceBind, onconnect, responding with 200 code');
    ...
    var socket = { service: self.service, pattern: self.pattern, type: 'bind', host: client_host, res: res, req: req };
    var i = addSocket(socket);


    var s = checkService(self.service, self.pattern);
    if ( s === false ) {
      //var i = addService({ name: service, pattern: pattern, queue: [], n: 0, xsettings: xsettings, self: self });     // add new service
      preview('checkService, false, service', self.service);
      return false;
    }
    ...
    //preview('send TEST BIND message now!!!!!!');
    //var testmessage = new Message();
    //testmessage.push('yo test BIND message!!!!');    // _0_ is a code picked up by the bind peer to start coms.
    //testmessage = testmessage.toBuffer();
    ...
    // recieving messages from connection peer.
  	...
  	req.on('data', function(chunk) {
			preview('bind got data, service x: ' + s + ' chunk', chunk.toString());	
		});
...
function newServiceConnect(service, pattern, query, self) {
  ...
  var i = checkService(self.service, self.pattern);
  if ( i === false ) {
    var i = addService({ name: self.service, type: 'connect', pattern: self.pattern, queue: [], n: 0, xsettings: xsettings, self: self });     // add new service
  }
  self.i = i;
  ...
  var sock = httpModule.request(options, function(res) {
    ...
    preview('newServiceConnect, connect', 'request, STATUS: ' + res.statusCode);
    ////preview('connect', 'request, HEADERS', res.headers);
    
    var sr = checkService(service, pattern);
    ...
    if ( res.statusCode === 200 ) {
      ...
      socket = { service: self.service, pattern: self.pattern, type: 'connect', host: options.hostname, res: sock, req: res };
  ...
  //preview('connect peer send TEST message now!!!!!!');
  //var testmessage = new Message();
  //testmessage.push('yo bind this is connect, test message!!!!');    // _0_ is a code picked up by the bind peer to start coms.
  //testmessage = testmessage.toBuffer();
  //sock.write(testmessage);
...
/*
	var corresponding_pattern = 'sub';
	switch(pattern) {
		case 'pub':
    	corresponding_pattern = 'sub';
			break;
		case 'sub':
			corresponding_pattern = 'pub';
			break;
		case 'push':
			corresponding_pattern = 'pull';
			break;
		case 'pull':
			corresponding_pattern = 'push';
			break;
		case 'chit':
			corresponding_pattern = 'chat';
			break;
		case 'chat':
			corresponding_pattern = 'chit';
			break;												
		default:
			corresponding_pattern = 'sub';
	}
*/
  ...
  for ( var s in sockets ) {
    if ( sockets[s].service === service && sockets[s].pattern === pattern ) {
    	//preview('socks.push: ' + sockets[s].service + ' sockets[s].pattern: ' + sockets[s].pattern + ' sockets[s].type: ' + sockets[s].type + ' s: ' + s);	
  ...
  if ( socks.length > 0 ) {
    if ( pattern === 'push' ) {
      ...
      // TODO: all these?
    } else if ( pattern === 'pub' || pattern === 'sub' || pattern === 'chit' || pattern === 'chat' ) {
      broadcast(i, socks, m);
    }
  } else {
...
function checkService(service, pattern) {
	preview('checkService, service:' + service + ', pattern:' + pattern);
  for ( var i in services ) {
  	//preview('checkService, services[i]', services[i]);
    if ( services[i].name === service && services[i].pattern === pattern ) {
      preview('checkService pid:' + process.pid + ', this service and pattern already exists, i: ' + i, ',name: ' + services[i].name + ', pattern: ' + pattern);
...
function broadcast(i, socks, m) {
  ...
  for ( var sock in socks ) {
  	preview('broadcast to sock: ' + sock + ' socks[sock].type: ' + socks[sock].type);
...
function addService(service) {
  ...
  preview('addService, i: ' + i + ', name: ' + service.name + ' pattern: ' + service.pattern);
