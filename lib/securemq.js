"use strict";
//
// securemq: Message-oriented secure service.
//
// Version: 0.0.1
...
function apikeys() {
	...
	// TODO:
	// expose so can be refreshed by caller, with list of valid apikeys
...
function newServiceBind(service, pattern) {
	...
	preview('newServiceBind, service: ' +  service + ', pattern: ' + pattern);
  ...
  if ( bound ) {
	...
	// TODO: should addService occur after authentication to avoild invalid services being added.
  ...
  function onconnect(req, res) {
    ...
    if ( _settings.secure ) {
      var header = req.headers['authorization']||'',        // get the header
        ...
        preview('authentication, listeners', server.listeners('authentication'));
        
        if ( Object.prototype.toString.call(_settings.apikey) === '[object Array]' ) {
        	for ( var ak in _settings.apikey ) {
        		if ( _settings.apikey[ak] === apikey ) {
        			authValid = true;
        			break;
        		}
        	}
        } else {
					if ( apikey.toString() === _settings.apikey.toString() ) {
						authValid = true;
					}
        }
        ...
        preview('authValid: ' + authValid + ' _settings.apikey: ' + _settings.apikey + ' apikey: ' + apikey);
        if ( !authValid ) {
    ...
    preview('connected event', {query: query, client_host: client_host, service: service, pattern: pattern});
    
    services[s].self.emit('connected', {query: query, client_host: client_host, service: service, pattern: pattern});    // emit connected and pass any url query objects.
...
function _send(socktype, service, pattern, i, m) {
  preview('_send, socketType: ' + socktype +  ' pattern: ' + pattern + ' m: ' + m + ' i: ' + i);
...
function broadcast(i, socks, m) {
  ...
  if ( services[i].xsettings.protocol === 'ldjson' ) {
    ...
    for ( var a in m ) {
    	...
    	// TODO: check if message is array
    	
    	if ( Object.prototype.toString.call(m[a]) === '[object Array]' ) {
    		var msg = new Message(m[a]);
    	} else {
    	 	var msg = new Message([m[a]]);
    	}
